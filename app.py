...
def register():
    if request.method == 'POST':
        ...
        new_user_id = new_user['id'] if new_user and 'id' in new_user else None
        conn.commit()
        cur.close()
        conn.close()
...
def chat():
    ...
    if conv_id:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute('SELECT * FROM ai_conversations WHERE id = %s AND user_id = %s', (conv_id, session['user_id']))
        full_conversation = cur.fetchone()
        cur.close()
        conn.close()
    ...
    return render_template('chat.html', conversations=recent_conversations, full_conversation=full_conversation)
...
def api_analytics():
    ...
    cur = conn.cursor()
    # Total AI sessions
    cur.execute('SELECT COUNT(*) FROM ai_conversations WHERE user_id = %s', (user_id,))
    total_ai_sessions = cur.fetchone()['count']
    # Breakdown by type
    cur.execute('''
    ...
    ''', (user_id,))
    ai_type_counts = {row['conversation_type']: row['count'] for row in cur.fetchall()}
    # Recent activity (last 10)
    cur.execute('''
        SELECT question, conversation_type, subject, created_at
        FROM ai_conversations
        WHERE user_id = %s
        ORDER BY created_at DESC
        LIMIT 10
    ''', (user_id,))
    recent_ai = cur.fetchall()
    # AI activity over time (last 30 days)
    cur.execute('''
    ...
    ''', (user_id,))
    ai_over_time = cur.fetchall()
    # Last active date
    cur.execute('''
        SELECT MAX(created_at) as last_active FROM ai_conversations WHERE user_id = %s
    ''', (user_id,))
    last_active = cur.fetchone()['last_active']
    # Average AI session duration (estimate: 2 min per session)
    avg_ai_duration = 2
    # Total AI time spent
    total_ai_time_spent = total_ai_sessions * avg_ai_duration
    cur.close()
    conn.close()
    return jsonify({
        ...
        'recent_ai': recent_ai,
        'ai_over_time': ai_over_time,
        'total_ai_time_spent': total_ai_time_spent,
